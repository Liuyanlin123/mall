...
import org.mochou.mymall.core.system.SystemConfig;
...
import static org.mochou.mymall.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static org.mochou.mymall.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    public Object update(@LoginUser Integer userId, @RequestBody MymallCart cart) {
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        MymallCart existCart = cartService.findById(id);
        if (existCart == null) {
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer grouponRulesId) {
        ...
        MymallGrouponRules grouponRules = grouponRulesService.queryById(grouponRulesId);
        ...
        for(MymallCouponUser couponUser : couponUserList){
            MymallCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), checkedGoodsPrice);
            if(coupon == null){
                continue;
            }
        ...
        if (couponId == null || couponId.equals(-1)){
            ...
            MymallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, checkedGoodsPrice);
            // 用户选择的优惠券有问题
            if(coupon == null){
                return ResponseUtil.badArgumentValue();
            }
